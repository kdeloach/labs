...
{	
	...
	public class TerritoryGen extends EventDispatcher
	{
		...
		// Holds next slot index to expand from for each territory. For example,
		// _expandSlotIndex[t0] = 0; means we will try to expand from the first slot of t0.
		// If slot0 on t0 has no neighbors left to expand to, we increase the counter.
		// If this counter is greater than the number of available slots, then t0 cannot expand any further.
		private var _expandSlotIndex:Dictionary;
		...
		public function gen():Promise
		{
			...
			_expandSlotIndex = new Dictionary();
		...
		public function expand():Promise
		{
			...
			for (var i:int = 0; i < _territories.length; i++) {
				...
				while (j++ < tsize) {
					var currSlot:Slot = currentSlot(territory);
		...
		// Each territory expands 1 slot at a time outward from the center.
		public function expand_old():Promise
		{
			var keepGoing:Boolean = false;
		...
		public function canExpand(t:Territory):Boolean
		{
			return _expandSlotIndex[t] < t.size();
		}
		
		// Meaning the current slot has no neighbors left to expand to so 
		// currentSlot() should return the next available slot.
		public function nextSlot(t:Territory):void
		{
			_expandSlotIndex[t] += 1;
		}
		
		public function resetSlot(t:Territory):void
		{
			_expandSlotIndex[t] = 0;
		}
		
		public function currentSlot(t:Territory):Slot
		{
			return t.slotAt(_expandSlotIndex[t]);
		}
